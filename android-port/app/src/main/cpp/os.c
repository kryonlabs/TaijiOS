...
emuinit(void *imod)
{
	...
	/* Clock application test - needs wmclient */
	static const char* test_modules[] = {
		"dis/clock.dis",       /* Clock application - test wmclient */
...
/*
 * genrandom - Generate random bytes for libmp/libsec
 * This is used by mprand() and various crypto functions
 */
void
genrandom(uchar *p, int n)
{
	...
	if(fd >= 0) {
		...
		/* Fallback: use clock_gettime for pseudo-random */
		for(int i = 0; i < n; i++) {
			p[i] = (uvlong)osfastticks() & 0xFF;
		}
	}
}

/*
 * truerand - Generate true random number for libsec
 * Uses genrandom() as the underlying source
 */
ulong
truerand(void)
{
